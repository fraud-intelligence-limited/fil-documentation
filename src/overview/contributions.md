...
If a peer deems a certain contribution to no longer be relevant (e.g., _it is a "false positive"_), they can [flag that contribution](../tutorials-api/flagging-a-contribution.md). Once a contribution is flagged, the ID of the peer that flagged it is saved with the contribution as `flagger` value.
...
| `fraudStatus`              | `string(enum)`    | The status of the event at the time of observing. <br> Can be one of the following: <ol><li>`Active`</li><li>`Expired`</li><li>`Flagged`</li></ol>                    |
